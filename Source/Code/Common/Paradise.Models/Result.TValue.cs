using System.Text.Json.Serialization;

namespace Paradise.Models;

/// <summary>
/// Represents an operation result that contains a value of type <typeparamref name="TValue"/>.
/// </summary>
/// <typeparam name="TValue">
/// The type of the value returned by the operation.
/// </typeparam>
public sealed class Result<TValue> : ResultBase
{
    #region Constructors
    /// <summary>
    /// Initializes a new instance of the <see cref="Result{TValue}"/> class.
    /// </summary>
    /// <param name="value">
    /// The value returned by the operation.
    /// </param>
    /// <param name="status">
    /// Indicates the result status. Provides more information on the nature of the errors, in case of any.
    /// <para>
    /// Provides more information on the nature of the errors,
    /// in case of any.
    /// </para>
    /// </param>
    public Result(TValue? value, OperationStatus status) : base(status)
        => Value = value;

    /// <summary>
    /// Initializes a new instance of the <see cref="Result{TValue}"/> class.
    /// </summary>
    /// <param name="status">
    /// Indicates the result status. Provides more information on the nature of the errors, in case of any.
    /// </param>
    /// <param name="error">
    /// Error code to create an <see cref="ApplicationError"/> instance.
    /// </param>
    /// <param name="arguments">
    /// Error description format arguments.
    /// </param>
    public Result(OperationStatus status, ErrorCode error, params object?[] arguments) : base(status, error, arguments) { }

    /// <summary>
    /// Initializes a new instance of the <see cref="Result{TValue}"/> class
    /// with the given result <paramref name="value"/> and <paramref name="errors"/>.
    /// </summary>
    /// <param name="status">
    /// Indicates the result status. Provides more information on the nature of the errors, in case of any.
    /// </param>
    /// <param name="errors">
    /// Errors associated with the result.
    /// </param>
    /// <param name="value">
    /// The value returned by the operation.
    /// </param>
    [JsonConstructor]
    public Result(OperationStatus status, IEnumerable<ApplicationError> errors, TValue? value) : base(status, errors)
        => Value = value;
    #endregion

    #region Properties
    /// <summary>
    /// The value returned by the operation.
    /// </summary>
    public TValue? Value { get; set; }
    #endregion
}